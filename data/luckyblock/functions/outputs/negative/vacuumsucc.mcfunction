...
# location assignment
execute if score tick lb_timer matches 1 as @e[tag=lb_breaklocation] run data merge entity @s {Tags:["vacuum_entity"]}
# main script here
...
execute if score tick lb_timer matches 2..200 at @e[tag=vacuum_entity] as @e[distance=5..25,type=player] at @s run tp @s ^ ^0.1 ^0.2 facing entity @e[sort=nearest,limit=1,tag=vacuum_entity]
# cleanup
